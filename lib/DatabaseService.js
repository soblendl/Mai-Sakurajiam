...
class DatabaseService {
    ...
    async load() {
        try {
            ...
            this.dbWorker = this.workerManager.getWorker('db', 'workers/dbWorker.js'); console.log('𖤐 Base de datos cargada');
            this.startAutoSave();
            return this.db;
        } catch (error) {
    ...
    updateUser(userId, updates) {
        ...
        if (result) {
            this.markDirty();
            console.log(`ദ്ദി ˉ͈̀꒳ˉ͈́ )✧ User ${userId} updated. New Balance: ${user.economy?.coins}`);
        } else {
            console.log(`ඞ Failed to update user ${userId}`);
        }
        return result;
    }
