...
export class MessageHandler {
    ...
    logDecoratedMessage(senderName, messageText) {
    ...
    async handleMessage(bot, m, isPrembot = false) {
        ...
        this.logDecoratedMessage(m.pushName || 'Usuario', text);
        ...
        try {
            ...
            console.error('ꕤ Error en handleMessage (antes de la cola):', error);
            const prefix = PREFIXES.find(p => text.startsWith(p));
            if (prefix && text.trim().length > prefix.length) {
                ...
                try {
                    ...
                    console.error('Error al enviar mensaje de error al usuario:', e);
                }
            }
        }
    }
